...
graduates.forEach(element => {
    return universities.push(element.university);
})
...
const contactInfo = [];
graduates.forEach(element => {
    return contactInfo.push(`${element.first_name} ${element.email}`)
})
...
zooAnimals.map(element => {
    return lowCaseAnimalNames.push(element.animal_name.toLowerCase());
})
...
zooAnimals.filter(element => {
    if (element.population < 5) {
        return lowPopulationAnimals.push(element);
    }
